...
import { Search, Filter, X, ChevronDown, Info, Home } from 'lucide-react';
...
interface PublicAntiNutrientContribution {
  id: string;
  flour_id: string;
  flour_name?: string;
  category_name?: string;
...
interface PublicEnzymeContribution {
  id: string;
  flour_id: string;
  flour_name?: string;
  category_name?: string;
...
function FlourCatalog() {
  ...
  const [antiNutrientContributions, setAntiNutrientContributions] = useState<Record<string, PublicAntiNutrientContribution>>({});
  const [enzymeContributions, setEnzymeContributions] = useState<Record<string, PublicEnzymeContribution>>({});
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      // Track page view
      trackPageView(user.id, 'flour_catalog');
    }
  }, [user]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Determine which tables to use based on subscription
        const categoriesTable = (!user || subscription?.tier === 'free') ? 'flour_categories_demo' : 'flour_categories';
        const floursTable = (!user || subscription?.tier === 'free') ? 'flours_demo' : 'flours';
        ...
        if (floursData) {
          // If user has paid subscription, fetch contribution data from public tables
          if (user && subscription?.tier !== 'free') {
            const flourIds = floursData.map(flour => flour.id);
            
            // Fetch anti-nutrient contributions
            const { data: antiNutrientData, error: antiNutrientError } = await supabase
              .from('publiccontributionanti_nutrients')
              .select('*')
              .in('flour_id', flourIds);
              
            if (antiNutrientError) throw antiNutrientError;
            
            // Create a map of flour ID to contribution data
            const antiNutrientMap: Record<string, PublicAntiNutrientContribution> = {};
            if (antiNutrientData) {
              antiNutrientData.forEach(contribution => {
                ...
                antiNutrientMap[contribution.flour_id] = {
                  ...contribution,
                  lectins,
                  tannins,
                  saponins,
            ...
            const enzymeMap: Record<string, PublicEnzymeContribution> = {};
            if (enzymeData) {
          ...
          setFlours(floursData.map(flour => ({
            ...flour,
            category: flour[categoriesTable]?.name || 'Non catégorisé'
          })));
        }
      } catch (error) {
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-green-800">
              {t('catalog.title')} {(!user || subscription?.tier === 'free') && `(${t('catalog.demoVersion')})`}
            </h1>
          </div>
        ...
        {(!user || subscription?.tier === 'free') && (
          <div className="bg-amber-50 border border-amber-200 p-4 rounded-lg">
            <p className="text-amber-800">
              {t('catalog.demoMessage')}{' '}
              <Link to="/pricing" className="text-amber-900 underline ml-1">{t('catalog.upgradeLink')}</Link>.
            </p>
          </div>
        )}
          ...
          {filteredFlours.map(flour => (
            <div key={flour.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-4">
                ...
                <button 
                  className="mt-4 w-full bg-green-700 text-white py-2 rounded hover:bg-green-600 transition-colors"
                  onClick={() => {
                    setSelectedFlour(flour);
                    // Track flour view if user is logged in
                    if (user) {
                      trackPageView(user.id, 'flour_details', {
                        flourId: flour.id,
                        ...
                        <>
                          ...
                          <div className="bg-green-50 p-4 rounded-lg mt-2">
                            ...
                            <p className="text-lg font-semibold text-green-800">
                              {enzymeContributions[selectedFlour.id].enzymes_total_contri?.toFixed(2) || '0.00'}
                            </p>
                          </div>
                        </>
                      ) : (
                      ...
                      {antiNutrientContributions[selectedFlour.id] ? (
                        <>
                          ...
                          <div className="bg-green-50 p-4 rounded-lg col-span-2">
                            ...
                            <p className="text-lg font-semibold text-green-800">
                              {antiNutrientContributions[selectedFlour.id].anti_nutrients_total_contri?.toFixed(2) || '0.00'}
                            </p>
                          </div>
                        </>
                      ) : (
