...
async function cancelPayPalSubscription(subscriptionId: string, accessToken: string): Promise<void> {
  const isProduction = Deno.env.get('PAYPAL_ENVIRONMENT') === 'production';
  const baseUrl = isProduction 
    ? 'https://api-m.paypal.com' 
    : 'https://api-m.sandbox.paypal.com';
...
serve(async (req) => {
  ...
  try {
    ...
    console.error('Error in paypal-cancel-subscription function:', error);
